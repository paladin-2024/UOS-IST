...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($idAgent <= 0 || $idSection <= 0) {
        echo "<script>
            Swal.fire({
                icon: 'error',
                title: 'Erreur',
                text: 'Agent et section sont obligatoires.'
            }).then(() => {
    ...
    if ($universite->checkDuplicateAgentSection($idAgent, $idSection)) {
        echo "<script>
            Swal.fire({
                icon: 'error',
                title: 'Erreur',
                text: 'Cet agent est déjà affecté à cette section.'
            }).then(() => {
                window.location.href = '../cours/enseignant';
            });
        </script>";
    ...
    // Call the function to add a section to an agent
    if ($universite->addAgentSection($idAgent, $idSection, $estPrincipal)) {
        echo "<script>
            Swal.fire({
                icon: 'success',
                title: 'Succès',
