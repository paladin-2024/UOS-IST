...
try {
    ...
    switch ($action) {
        case 'progress':
            ...
            $progress = isset($_SESSION['fiches_progress_' . $importId]) ? $_SESSION['fiches_progress_' . $importId] : 0;
            $current = isset($_SESSION['fiches_current_' . $importId]) ? $_SESSION['fiches_current_' . $importId] : 0;
        ...
        case 'start':
            ...
            $_SESSION['fiches_progress_' . $importId] = 0;
            $_SESSION['fiches_current_' . $importId] = 0;
            
            // Créer le dossier de destination
            $outputDir = dirname(__DIR__) . '/temp/fiches_' . $importId . '_' . date('Y-m-d_H-i-s');
            if (!is_dir($outputDir)) {
                mkdir($outputDir, 0755, true);
            }
            
            $_SESSION['fiches_output_dir_' . $importId] = $outputDir;
            ...
            echo json_encode([
                'success' => true,
                'total' => $totalEtudiants,
                'message' => 'Génération des fiches initialisée'
            ]);
            exit;

        case 'generate':
            // Générer les fiches une par une
            $outputDir = isset($_SESSION['fiches_output_dir_' . $importId]) ? $_SESSION['fiches_output_dir_' . $importId] : null;
            ...
            for ($i = $current; $i < min($current + $batchSize, $totalEtudiants); $i++) {
                ...
                try {
                    // Log pour debug
                    error_log("Génération fiche pour: " . $matricule);
                    
                    // Générer le PDF directement avec TCPDF (même logique que export_bulletin_individuel_ancien.php)
                    $filename = $outputDir . '/Fiche_Validation_' . $matricule . '.pdf';
                    generateFichePDF($importId, $matricule, $filename, $grilleAncienne, $universite);
                    ...
                    $generated++;
                    error_log("Fiche générée avec succès pour: " . $matricule);
                    
                } catch (Exception $e) {
                    $errorMsg = "Erreur pour {$matricule}: " . $e->getMessage();
                    $errors[] = $errorMsg;
                    error_log("ERREUR génération fiche: " . $errorMsg);
                }
            }
            ...
            $_SESSION['fiches_current_' . $importId] = $newCurrent;
            $_SESSION['fiches_progress_' . $importId] = $progress;

            $isComplete = ($newCurrent >= $totalEtudiants);

            if ($isComplete) {
                // Créer un ZIP avec tous les PDFs
                $zipFile = dirname($outputDir) . '/Fiches_Validation_' . $importInfo['promotion'] . '_' . date('Y-m-d_H-i-s') . '.zip';
                createZipFromDirectory($outputDir, $zipFile);
                
                $_SESSION['fiches_zip_' . $importId] = $zipFile;
        ...
        case 'download':
            ...
            $zipFile = isset($_SESSION['fiches_zip_' . $importId]) ? $_SESSION['fiches_zip_' . $importId] : null;
            ...
            unset($_SESSION['fiches_progress_' . $importId]);
            unset($_SESSION['fiches_current_' . $importId]);
            unset($_SESSION['fiches_output_dir_' . $importId]);
            unset($_SESSION['fiches_zip_' . $importId]);
    ...
    $logMessage = "ERREUR EXPORT FICHES ANCIENNES - " . date('Y-m-d H:i:s') . "\n";
    ...
    error_log($logMessage, 3, dirname(__DIR__) . '/debug_export_fiches.log');
    error_log("ERREUR EXPORT FICHES: " . $e->getMessage());
...
/**
 * Générer le PDF de la fiche de validation pour un étudiant avec TCPDF (identique à export_bulletin_individuel_ancien.php)
 */
function generateFichePDF($importId, $matricule, $filename, $grilleAncienne, $universite) {
    try {
        ...
        $pdf->SetTitle('Bulletin Individuel - ' . ($etudiant['noms'] ?? 'Étudiant'));
        ...
        $qrData = "BULLETIN-" . $etudiant['matricule'] . "-" . str_replace(' ', '_', $importInfo['promotion']) . "-" . date('dmY');
        ...
        error_log("Erreur génération PDF fiche: " . $e->getMessage());
        throw $e;
    }
}
