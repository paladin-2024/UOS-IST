...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($structure->updateLigneRecette($idLigne, $codeLigne, $designation, $montant)) {
        echo "<script>
            Swal.fire({
                icon: 'success',
                title: 'Succès',
                text: 'Ligne de récette mise à jour avec succès.'
            }).then(() => {
                window.location.href = '../budget/budget.recette.grp.edit';
            });
        </script>";
        ...
        echo "<script>
            Swal.fire({
                icon: 'error',
                title: 'Erreur',
                text: 'Erreur lors de la mise à jour de la ligne de récette.'
            }).then(() => {
                window.location.href = '../budget/budget.recette.grp.edit';
            });
        </script>";
