...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    try {
        ...
        $code_fournisseur = isset($_POST['code_fournisseur']) ? trim($_POST['code_fournisseur']) : '';
        ...
        if (empty($code_fournisseur) || empty($nom_fournisseur) || $id_compte_comptable <= 0) {
            throw new Exception("Les champs Code, Nom et Compte comptable sont obligatoires.");
        }
        
        // Vérifier si le code existe déjà
        $stmt = $db->prepare("SELECT COUNT(*) FROM fournisseur WHERE code_fournisseur = :code_fournisseur");
        $stmt->bindParam(':code_fournisseur', $code_fournisseur, PDO::PARAM_STR);
        $stmt->execute();
        
        if ($stmt->fetchColumn() > 0) {
            throw new Exception("Ce code fournisseur existe déjà. Veuillez en choisir un autre.");
        }
        ...
        $stmt = $db->prepare("INSERT INTO fournisseur 
            (code_fournisseur, nom_fournisseur, adresse, telephone, email, nif, rccm, 
            id_compte_comptable, delai_paiement, actif, id_user_creation) 
            VALUES 
            (:code_fournisseur, :nom_fournisseur, :adresse, :telephone, :email, :nif, :rccm, 
            :id_compte_comptable, :delai_paiement, :actif, :id_user_creation)");
        ...
        $logStmt = $db->prepare("INSERT INTO log_operation 
            ...
            VALUES 
            (:id_user, 'création', 'fournisseur', :id_enregistrement, :description, :adresse_ip, :navigateur)");
        
        $description = "Création du fournisseur: $nom_fournisseur (Code: $code_fournisseur)";
        $adresse_ip = $_SERVER['REMOTE_ADDR'];
        $navigateur = $_SERVER['HTTP_USER_AGENT'];
        
        $logStmt->bindParam(':id_user', $id_user_creation, PDO::PARAM_INT);
        $logStmt->bindParam(':id_enregistrement', $idFournisseur, PDO::PARAM_INT);
        ...
        echo "<script>
            Swal.fire({
                ...
                window.location.href = '../fournisseurs/fournisseurs.add';
            });
        </script>";
        exit;
    }
