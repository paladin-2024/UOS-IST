...
function debug_to_file($data, $label = '') {
    ...
    file_put_contents('../debug_bulletin_ancien.log', $output, FILE_APPEND);
}
...
$bulletinUrl = $baseUrl . "/controller/export_bulletin_individuel_ancien.php?matricule=" . $matricule .
               "&import_id=" . $importId;
...
try {
    ...
    // Récupérer les UEs avec leurs notes pour affichage détaillé
    ...
    foreach ($uesAvecEcues as $ue) {
        ...
        if ($hasAllNotes) {
            ...
            if ($totalCoeffUE > 0) {
                ...
                $ue['moyenne'] = $moyenneUE;
                $ue['est_validee'] = $moyenneUE >= 10 ? 1 : 0;
    ...
    // Générer le contenu HTML du bulletin (EXACTEMENT comme l'original)
    $html = '
<!DOCTYPE html>
<html lang="fr">
<head>
    <meta charset="UTF-8">
    <title>Bulletin - ' . htmlspecialchars($etudiant['noms']) . '</title>
    <style>
        body {
...
<body>
    ...
    <div class="header">
        <h1>FICHE DE VALIDATION DE CREDIT</h1>
    ...
    // Afficher les semestres (pour les grilles anciennes, on simule un semestre unique)
    $html .= '<h3>' . htmlspecialchars($importInfo['semestre']) . '</h3>';
    
    // Tableau des notes par UE/ECUE (structure identique à export_releve_notes_ancienne.php)
    $html .= '<table>
<thead>
    <tr>
...
<tbody>';
    ...
    // Pour chaque UE
    foreach ($uesAvecEcues as $ueData) {
        $estValidee = isset($ueData['est_validee']) && $ueData['est_validee'] == 1;
        $moyenneUE = isset($ueData['moyenne']) ? $ueData['moyenne'] : null;
        $moyenneClass = ($moyenneUE !== null && floatval($moyenneUE) < 10) ? 'text-danger' : '';
        $validationClass = $estValidee ? 'text-success' : 'text-danger';
        
        $html .= '<tr class="bg-light">
            <td class="text-left"><strong>' . htmlspecialchars($ueData['code_ue'] ?? 'UE') . ' - ' . htmlspecialchars($ueData['nom_ue']) . '</strong></td>
            <td>' . (isset($ueData['credits_ue']) ? number_format(floatval($ueData['credits_ue']), 1) : '-') . '</td>
        ...
        foreach ($ueData['ecues'] as $ecue) {
            ...
            $html .= '<tr>
                <td class="text-left">&nbsp;&nbsp;&nbsp;&nbsp;' . htmlspecialchars($ecue['nom_ecue']) . '</td>
                <td>' . (isset($ecue['credits']) ? number_format(floatval($ecue['credits']), 1) : '-') . '</td>
    ...
    // Ajouter un résumé pour le semestre
    $html .= '<div style="text-align: right; font-size: 9pt; margin-bottom: 10px;">';
    
    if ($notesManquantes) {
        $html .= '<strong>Notes incomplètes | Crédits validés: ' . 
            $totalCreditsValides . '/' . $totalCredits . '</strong>';
    } else {
        $html .= '<strong>Moyenne: ' . number_format($moyenneGenerale, 2) . ' | Crédits validés: ' . 
            $totalCreditsValides . '/' . $totalCredits . ' (' . 
            number_format(($moyenneGenerale / 20) * 100, 2) . '%)</strong>';
    }
    ...
    // Logique de décision pour grilles anciennes
    if ($notesManquantes) {
    ...
    // Ajouter une div pour contenir le QR code et le footer (EXACTE de l'original)
    ...
    // Footer avec la date d'impression (EXACT de l'original)
    ...
    // Générer le PDF (EXACT de l'original)
    $html2pdf = new Html2Pdf('P', 'A4', 'fr', true, 'UTF-8', array(10, 10, 10, 10));
    $html2pdf->writeHTML($html);
    
    $filename = 'Fiche_Validation_' . $matricule . '_' . date('Y-m-d_H-i-s') . '.pdf';
