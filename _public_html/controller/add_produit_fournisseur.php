...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    try {
        ...
        if ($id_fournisseur <= 0 || $id_produit <= 0 || $prix_achat <= 0) {
            throw new Exception("Tous les champs obligatoires doivent être remplis correctement.");
        }
        
        // Vérifier si ce produit est déjà associé à ce fournisseur
        $stmt = $db->prepare("SELECT COUNT(*) FROM produit_fournisseur 
                             WHERE id_produit = :id_produit AND id_fournisseur = :id_fournisseur");
        $stmt->bindParam(':id_produit', $id_produit, PDO::PARAM_INT);
        $stmt->bindParam(':id_fournisseur', $id_fournisseur, PDO::PARAM_INT);
        $stmt->execute();
        
        if ($stmt->fetchColumn() > 0) {
            throw new Exception("Ce produit est déjà associé à ce fournisseur.");
        }
        ...
        $stmt = $db->prepare("INSERT INTO produit_fournisseur 
                             (id_produit, id_fournisseur, prix_achat, delai_livraison, 
                             est_fournisseur_principal, id_user_creation) 
                             VALUES 
                             (:id_produit, :id_fournisseur, :prix_achat, :delai_livraison, 
                             :est_fournisseur_principal, :id_user_creation)");
        ...
        $stmt->execute();
                ...
                $logStmt = $db->prepare("INSERT INTO log_operation 
                    ...
                    VALUES 
                    (:id_user, 'création', 'produit_fournisseur', :id_enregistrement, :description, :adresse_ip, :navigateur)");
                
                $idProduitFournisseur = $db->lastInsertId();
                $description = "Association du produit '$nomProduit' au fournisseur '$nomFournisseur' avec prix: $prix_achat USD";
                $adresse_ip = $_SERVER['REMOTE_ADDR'];
                $navigateur = $_SERVER['HTTP_USER_AGENT'];
                
                $logStmt->bindParam(':id_user', $id_user_creation, PDO::PARAM_INT);
                $logStmt->bindParam(':id_enregistrement', $idProduitFournisseur, PDO::PARAM_INT);
                ...
                echo "<script>
                    Swal.fire({
                        title: 'Succès',
                        text: 'Le produit a été associé au fournisseur avec succès.',
                        icon: 'success',
