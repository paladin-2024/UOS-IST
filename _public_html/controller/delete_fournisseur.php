<?php
session_start();
error_reporting(E_ALL); ini_set("display_errors", 1);
require_once dirname(__DIR__) . '/views/405.php';
require_once dirname(__DIR__) . '/config/Connexion.php';

// Vérifier si l'utilisateur est connecté
if (!isset($_SESSION['id'])) {
    header('Location: ../login');
    exit();
}

// Initialiser la connexion
$db = Connexion::getInstance()->getPDO();

if (isset($_GET['id'])) {
    try {
        $id_fournisseur = intval($_GET['id']);
        
        if ($id_fournisseur <= 0) {
            throw new Exception("Identifiant invalide.");
        }
        
        // Vérifier que le fournisseur existe
        $stmt = $db->prepare("SELECT nom_fournisseur FROM fournisseur WHERE id_fournisseur = :id");
        $stmt->bindParam(':id', $id_fournisseur, PDO::PARAM_INT);
        $stmt->execute();
        $nomFournisseur = $stmt->fetchColumn();
        
        if (!$nomFournisseur) {
            throw new Exception("Ce fournisseur n'existe pas.");
        }
        
        // Vérifier s'il a des commandes
        $stmt = $db->prepare("SELECT COUNT(*) FROM commande_fournisseur WHERE id_fournisseur = :id");
        $stmt->bindParam(':id', $id_fournisseur, PDO::PARAM_INT);
        $stmt->execute();
        
        if ($stmt->fetchColumn() > 0) {
            throw new Exception("Impossible de supprimer ce fournisseur car il possède des commandes associées.");
        }
        
        // Vérifier s'il a des factures
        $stmt = $db->prepare("SELECT COUNT(*) FROM facture_fournisseur WHERE id_fournisseur = :id");
        $stmt->bindParam(':id', $id_fournisseur, PDO::PARAM_INT);
        $stmt->execute();
        
        if ($stmt->fetchColumn() > 0) {
            throw new Exception("Impossible de supprimer ce fournisseur car il possède des factures associées.");
        }
        
        // Supprimer les associations avec les produits
        $stmt = $db->prepare("DELETE FROM produit_fournisseur WHERE id_fournisseur = :id");
        $stmt->bindParam(':id', $id_fournisseur, PDO::PARAM_INT);
        $stmt->execute();
        
        // Supprimer le fournisseur
        $stmt = $db->prepare("DELETE FROM fournisseur WHERE id_fournisseur = :id");
        $stmt->bindParam(':id', $id_fournisseur, PDO::PARAM_INT);
        $stmt->execute();
        
        // Journalisation de l'action
        $id_user = $_SESSION['id'];
        $logStmt = $db->prepare("INSERT INTO log_operation 
            (id_user, type_operation, table_concernee, id_enregistrement, description, adresse_ip, navigateur) 
            VALUES 
            (:id_user, 'suppression', 'fournisseur', :id_enregistrement, :description, :adresse_ip, :navigateur)");
        
        $description = "Suppression du fournisseur: $nomFournisseur";
        $adresse_ip = $_SERVER['REMOTE_ADDR'];
        $navigateur = $_SERVER['HTTP_USER_AGENT'];
        
        $logStmt->bindParam(':id_user', $id_user, PDO::PARAM_INT);
        $logStmt->bindParam(':id_enregistrement', $id_fournisseur, PDO::PARAM_INT);
        $logStmt->bindParam(':description', $description, PDO::PARAM_STR);
        $logStmt->bindParam(':adresse_ip', $adresse_ip, PDO::PARAM_STR);
        $logStmt->bindParam(':navigateur', $navigateur, PDO::PARAM_STR);
        
        $logStmt->execute();
        
        // Redirection avec message de succès
        echo "<script>
            Swal.fire({
                title: 'Succès',
                text: 'Le fournisseur a été supprimé avec succès.',
                icon: 'success',
                confirmButtonText: 'OK'
            }).then((result) => {
                window.location.href = '../fournisseurs/fournisseurs.list';
            });
        </script>";
        exit;
        
    } catch (Exception $e) {
        echo "<script>
            Swal.fire({
                title: 'Erreur',
                text: '" . addslashes($e->getMessage()) . "',
                icon: 'error',
                confirmButtonText: 'OK'
            }).then((result) => {
                window.location.href = '../fournisseurs/fournisseurs.list';
            });
        </script>";
        exit;
    }
} else {
    // Redirection si accès direct au fichier
    echo "<script>
        Swal.fire({
            title: 'Erreur',
            text: 'Paramètre manquant',
            icon: 'error',
            confirmButtonText: 'OK'
        }).then((result) => {
            window.location.href = '../fournisseurs/fournisseurs.list';
        });
    </script>";
    exit;
}
