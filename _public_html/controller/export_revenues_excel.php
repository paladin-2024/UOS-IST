...
// Récupérer les recettes périodiques
$revenues = $structureModel->getPeriodicRevenues($structureId, $startDate, $endDate);

// Trier par motif
usort($revenues, function ($a, $b) {
    return strcmp($a['motif'], $b['motif']);
});
...
// Titre du rapport
$sheet->setCellValue('A1', 'Rapport des recettes du ' . date('d/m/Y'));
...
// En-têtes du tableau
$headers = ['Motif', 'Montant', 'Dépositaire', 'Date d\'Opération', 'Date d\'Enregistrement'];
...
foreach ($revenues as $revenue) {
    $data = [
        ...
        $revenue['montantR'],
        $revenue['depositaire'],
        date('d/m/Y', strtotime($revenue['dateOperation'])),
        date('d/m/Y H:i:s', strtotime($revenue['dateEnregistrement']))
    ];
    ...
    $totalAmount += $revenue['montantR']; // Accumuler les montants
    $row++;
}
...
// Nom du fichier
$fileName = 'revenues_report_' . date('Y-m-d_H-i-s') . '.xlsx';
