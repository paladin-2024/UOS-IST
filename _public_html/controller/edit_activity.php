...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($activityId <= 0 || empty($intitule) || empty($dateDebut) || empty($dateFin) || $budget <= 0 || empty($etatActivite) || $projetId <= 0) {
        echo "<script>
            Swal.fire({
                icon: 'error',
                title: 'Erreur',
    ...
    // Mise à jour de l'activité via le modèle
    try {
        $data = [
            ':idActivite_projet' => $activityId,
            ':intitule' => $intitule,
            ':dateDebut' => $dateDebut,
            ':dateFin' => $dateFin,
            ':budget' => $budget,
        ...
        echo "<script>
            Swal.fire({
                icon: 'error',
                title: 'Erreur',
                text: 'Erreur lors de la mise à jour de l\'activité: " . addslashes($e->getMessage()) . "'
            }).then(() => {
                window.location.href = '../projet/activite.add';
            });
        </script>";
