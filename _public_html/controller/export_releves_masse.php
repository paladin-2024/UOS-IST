...
function debug_to_file($data, $label = '') {
    ...
    file_put_contents('../debug_releves_masse.log', $output, FILE_APPEND);
}
...
$pdf->SetTitle('Relevés de Notes - ' . $promotion['designationPromotion']);
$pdf->SetSubject('Relevés de notes officiels');
$pdf->SetKeywords('Étudiants, Notes, Relevés, Officiels');
...
// Pour chaque étudiant, générer son relevé
foreach ($etudiants as $etudiant) {
    ...
    // Pré-calculer la décision pour le QR code
    $decisionText = '';
    $mentionEtatText = '-';
    ...
    if ($responsableSection) {
        $pdf->SetFont('helvetica', 'B', 6);
        $pdf->Cell(50, 3, $responsableSection['noms'], 0, 1, 'R');
    } else {
    ...
    // Footer avec informations de l'université
...
// Générer le PDF
$filename = 'Releves_' . $promotionId . '_' . $sessionId . '_' . date('Ymd') . '.pdf';
