...
// Fetch recette budgets accessible by the user
...
<head>
    <meta charset="UTF-8">
    <title>Budgets de Recettes</title>
    <style>
...
foreach ($budgets as $budget) {
    ...
    $groupes = $structureModel->getGroupesRecetteByUserAccess2($userId, $annee, $limit);
    foreach ($groupes as $groupe) {
        if ($groupe['Budget_recette_structure_idBudget_recette_structure'] == $budget['idBudget_recette_structure']) {
            $totalBudget += $groupe['soldeGR'];
            $html .= '<tr>
                <td><strong>' . htmlspecialchars($groupe['designationGR']) . '</strong></td>
                <td colspan="2"></td>
                <td style="text-align: right;"><strong>' . number_format($groupe['soldeGR'], 2, ',', ' ') . ' USD</strong></td>
            </tr>';

            $lignes = $structureModel->getLignesRecetteByGroupe($groupe['idGroupe_recette_structure']);
...
try {
    ...
    $html2pdf->output('budgets_recettes.pdf', 'I');
} catch (Exception $e) {
