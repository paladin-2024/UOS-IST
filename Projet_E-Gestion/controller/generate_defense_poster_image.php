...
try {
    ...
    // Vérifier que Imagick est disponible
    if (!extension_loaded('imagick')) {
        throw new Exception('Extension Imagick non disponible sur le serveur');
    }
    ...
    // ==============================
    // GÉNÉRATION DU PDF (même code que generate_defense_poster.php)
    // ==============================
    ...
    $couleurPrimaire = [25, 42, 86];
    $couleurSecondaire = [64, 115, 158];
    $couleurAccent = [212, 175, 55];
    $couleurTexte = [33, 33, 33];
    $couleurBlanc = [255, 255, 255];
    $couleurGrisClair = [245, 245, 250];
    ...
    $tempPdfPath = sys_get_temp_dir() . '/soutenance_' . $soutenanceId . '_' . uniqid() . '.pdf';
    file_put_contents($tempPdfPath, $pdfContent);
    
    // Convertir avec Imagick
    $imagick = new Imagick();
    $imagick->setResolution(300, 300); // Haute résolution pour qualité
    $imagick->readImage($tempPdfPath . '[0]'); // Première page
    ...
    // Fond blanc (pour PNG transparent)
    ...
    header('Content-Disposition: inline; filename="affiche_soutenance_' . $soutenanceId . '.' . $extension . '"');
    ...
    error_log('Erreur generate_defense_poster_image.php: ' . $e->getMessage());
    
    // Retourner une erreur JSON si ce n'est pas une requête d'image
