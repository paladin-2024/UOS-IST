...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    try {
        ...
        $stmt = $db->prepare("SELECT id_compte FROM compte_comptable WHERE numero_compte = :numero_compte AND id_compte != :id_compte");
        $stmt->bindParam(':numero_compte', $numero_compte);
        $stmt->bindParam(':id_compte', $id_compte, PDO::PARAM_INT);
        $stmt->execute();
        
        if ($stmt->rowCount() > 0) {
            throw new Exception("Ce numéro de compte existe déjà pour un autre compte.");
        }
        
        // Mettre à jour le compte
        $stmt = $db->prepare("UPDATE compte_comptable 
                              SET numero_compte = :numero_compte, 
                                  intitule_compte = :intitule_compte, 
                                  classe_compte = :classe_compte, 
        ...
        $_SESSION['success_message'] = "Compte comptable modifié avec succès.";
