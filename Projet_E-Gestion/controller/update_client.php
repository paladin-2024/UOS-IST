...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    try {
        ...
        $clientActuel = $checkStmt->fetch(PDO::FETCH_ASSOC);
        ...
        // Vérifier si le code existe déjà pour un autre client
        $stmt = $db->prepare("SELECT COUNT(*) FROM client WHERE code_client = :code_client AND id_client != :id_client");
        ...
        $logStmt = $db->prepare("INSERT INTO log_operation 
            ...
            VALUES 
            (:id_user, 'modification', 'client', :id_enregistrement, :description, :adresse_ip, :navigateur)");
        
        $description = "Modification du client: $nom_client (Code: $code_client)";
        ...
        echo "<script>
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href = '../clients/clients.view&id=$id_client';
                }
            });
        </script>";
        exit;
        ...
        echo "<script>
            Swal.fire({
                ...
                window.location.href = '../clients/clients.edit&id=" . (isset($id_client) ? $id_client : 0) . "';
            });
        </script>";
        exit;
    }
