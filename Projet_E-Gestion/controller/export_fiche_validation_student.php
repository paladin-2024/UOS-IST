...
if (!isset($_SESSION['student_id'])) {
    header('Location: ../views/portail/login');
    exit();
}
...
function debug_to_file($data, $label = '') {
    ...
    file_put_contents('../debug_fiche_validation_student.log', $output, FILE_APPEND);
}

// Paramètres - matricule depuis la session
...
try {
    ...
    // Signature électronique du chef de section
    if (!empty($importInfo['chef_section_signature'])) {
        $signaturePath = dirname(__DIR__) . '/uploads/' . $importInfo['chef_section_signature'];
        if (file_exists($signaturePath)) {
            $pdf->Image($signaturePath, $signX + 15, $signY + 4, 25, 0, '', '', '', false, 200);
        }
    }
    ...
    $footerText = 'Imprimé par ' . htmlspecialchars($etudiant['noms']) . ', le ' . date('d/m/Y');
...
// Pas de balise de fermeture pour éviter toute sortie accidentelle
