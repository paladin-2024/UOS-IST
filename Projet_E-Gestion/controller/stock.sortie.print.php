...
if (isset($_GET['id']) && intval($_GET['id']) > 0) {
    ...
    try {
        ...
        $stmtSortie->execute();
        $sortie = $stmtSortie->fetch(PDO::FETCH_ASSOC);

        if (!$sortie) {
            throw new Exception("Sortie de stock introuvable");
        }
        ...
        $queryDetails = "SELECT d.*, p.code_produit, p.libelle_produit, u.code_unite, u.symbole_unite,
                        ...
                        LEFT JOIN detail_sortie_lot dl ON d.id_detail_sortie = dl.id_detail_sortie
                        LEFT JOIN lot_produit l ON dl.id_lot = l.id_lot
                        WHERE d.id_sortie = :id_sortie
                        ORDER BY p.libelle_produit";
        $stmtDetails = $db->prepare($queryDetails);
        $stmtDetails->bindParam(':id_sortie', $idSortie, PDO::PARAM_INT);
        $stmtDetails->execute();
        ...
        $pdf->SetTitle('Bon de sortie de stock');
        $pdf->SetSubject('Bon de sortie de stock');
        $pdf->SetKeywords('Stock, Sortie, Bon');
        ...
        /*
        $primaryColor = array(140, 0, 0); // Rouge foncé
        $secondaryColor = array(180, 70, 70); // Rouge plus clair
        $accentColor = array(194, 0, 0); // Rouge moyen*/
        ...
        function generatePage($pdf, $sortie, $produitsGroupes, $configInstitution, $primaryColor, $secondaryColor, $accentColor, $isDuplicata = false)
        {
            ...
            $pdf->Ln(5);
            $pdf->Cell(0, 10, 'BON DE SORTIE DE STOCK', 0, 1, 'C', 1);

            $pdf->SetTextColor(80, 80, 80);
            $pdf->SetFont('helvetica', 'B', 10);
            $pdf->Cell(0, 8, 'N° ' . $sortie['numero_sortie'], 0, 1, 'C');

            // Préparer les données du QR Code
            $qrCodeData = "BON DE SORTIE DE STOCK\n";
            $qrCodeData .= "N°: " . $sortie['numero_sortie'] . "\n";
            $qrCodeData .= "Date: " . date('d/m/Y', strtotime($sortie['date_sortie'])) . "\n";
            $qrCodeData .= "Dépôt: " . $sortie['libelle_depot'] . "\n";
            $qrCodeData .= "Type: " . $sortie['type_sortie'] . "\n";
            ...
            $pdf->Ln(-5);
                        ...
                        $pdf->SetFont('helvetica', 'B', 12);
                        $pdf->Cell(0, 8, 'INFORMATIONS DE LA SORTIE', 0, 1, 'L');
                        ...
                        $pdf->Cell(30, 5, 'N° Sortie:', 0, 0, 'L');
                        $pdf->SetFont('helvetica', '', 9);
                        $pdf->Cell(65, 5, $sortie['numero_sortie'], 0, 0, 'L');
                        
                        // Colonne droite
                        $pdf->SetFont('helvetica', 'B', 9);
                        $pdf->Cell(30, 5, 'Date sortie:', 0, 0, 'L');
                        $pdf->SetFont('helvetica', '', 9);
                        $pdf->Cell(0, 5, date('d/m/Y', strtotime($sortie['date_sortie'])), 0, 1, 'L');
            
                        // Deuxième ligne
                        $pdf->SetFont('helvetica', 'B', 9);
                        $pdf->Cell(30, 5, 'Dépôt:', 0, 0, 'L');
                        $pdf->SetFont('helvetica', '', 9);
                        $pdf->Cell(65, 5, $sortie['libelle_depot'], 0, 0, 'L');
                        
                        $pdf->SetFont('helvetica', 'B', 9);
                        $pdf->Cell(30, 5, 'Type de sortie:', 0, 0, 'L');
                        $pdf->SetFont('helvetica', '', 9);
                        $pdf->Cell(0, 5, $sortie['type_sortie'] . ' | Statut: ' . $sortie['etat'], 0, 1, 'L');
            
                        // Troisième ligne
                        $pdf->SetFont('helvetica', 'B', 9);
                        $pdf->Cell(30, 5, 'Référence:', 0, 0, 'L');
                        $pdf->SetFont('helvetica', '', 9);
                        $pdf->Cell(0, 5, $sortie['reference_document'] ?: '-', 0, 1, 'L');
                        ...
                        if (!empty($sortie['observation'])) {
                            ...
                            $pdf->SetFont('helvetica', '', 9);
                            $pdf->MultiCell(0, 5, $sortie['observation'], 0, 'L');
                        }
                        ...
                        // MODIFICATION 2: Tableau des signatures avec trois colonnes incluant le destinataire
                        $pdf->SetTextColor($secondaryColor[0], $secondaryColor[1], $secondaryColor[2]);
                        $pdf->SetFont('helvetica', 'B', 12);
                        ...
                        $pdf->Cell($colWidth, 6, 'Destinataire', 1, 0, 'C', 1);
                        $pdf->Cell($colWidth, 6, 'Approuvé par', 1, 1, 'C', 1);
                        
                        // Contenu du tableau - Ligne 1 (noms)
                        $pdf->SetFont('helvetica', '', 9);
                        $pdf->Cell($colWidth, 10, $sortie['user_creation'] ?? 'N/A', 1, 0, 'C');
                        $pdf->Cell($colWidth, 10, isset($sortie['destinataire']) ? $sortie['destinataire'] : '........................', 1, 0, 'C');
                        $pdf->Cell($colWidth, 10, $sortie['user_validation'] ?? 'N/A', 1, 1, 'C');
                        
                        // Ligne 2 (dates)
                        $pdf->SetFont('helvetica', 'I', 8);
                        $pdf->Cell($colWidth, 5, 'Date: ' . date('d/m/Y', strtotime($sortie['date_creation'])), 1, 0, 'C');
                        $pdf->Cell($colWidth, 5, 'Date: ........................', 1, 0, 'C');
                        $pdf->Cell($colWidth, 5, 'Date: ' . ($sortie['date_validation'] ? date('d/m/Y', strtotime($sortie['date_validation'])) : 'N/A'), 1, 1, 'C');
                    ...
                    generatePage($pdf, $sortie, $produitsGroupes, $configInstitution, $primaryColor, $secondaryColor, $accentColor, $isDuplicata = false);
                    ...
                    // Outputting the PDF
                    $pdf->Output('Bon_Sortie_Stock_' . $sortie['numero_sortie'] . '.pdf', 'I');
                } catch (Exception $e) {
                    die("Erreur: " . $e->getMessage());
                }
            } else {
                die("ID de sortie de stock non spécifié");
            }
