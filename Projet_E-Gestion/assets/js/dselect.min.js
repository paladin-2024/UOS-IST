  ...
  e.style.display = "none"; const a = "dselect-wrapper", d = "dselect-no-results", o = "dselect-tag", f = "dselect-tag-remove", i = "dselect-placeholder", m = "dselect-clear", x = "dselect-clearable", p = !1, S = !1, c = !1, h = "360px", g = "", A = w("search") || t.search || p, k = w("creatable") || t.creatable || S, y = w("clearable") || t.clearable || c, M = e.dataset.dselectMaxHeight || t.maxHeight || h; let E = e.dataset.dselectSize || t.size || g; E = E !== "" ? ` form-select-${E}` : ""; const b = `form-select${E}`, H = A ? `<input onkeydown="return event.key !== 'Enter'" onkeyup="dselectSearch(event, this, '${a}', '${b}', ${k})" type="text" class="form-control" placeholder="Recherchez ici..." autofocus>` : ""; function w(n) { const l = `data-dselect-${n}`; return e.hasAttribute(l) ? e.getAttribute(l).toLowerCase() === "true" : null } function B() { e.nextElementSibling && e.nextElementSibling.classList && e.nextElementSibling.classList.contains(a) && e.nextElementSibling.remove() } function $(n) { return n.getAttribute("value") === "" } function L(n, l) {
    if (l) {
      const s = Array.from(n).filter(r => r.selected && !$(r)), v = Array.from(n).filter(r => $(r)); let u = []; if (s.length === 0) { const r = v.length ? v[0].textContent : "&nbsp;"; u.push(`<span class="${i}">${r}</span>`) } else for (const r of s) u.push(`
          ...
          `); return u.join("")
    } else { const s = n[n.selectedIndex]; return $(s) ? `<span class="${i}">${s.innerHTML}</span>` : s.innerHTML }
  } function C(n) { const l = n[n.selectedIndex]; return $(l) ? "" : l.textContent } function T(n) { let l = []; for (const s of n) if (s.tagName === "OPTGROUP") l.push(`<h6 class="dropdown-header">${s.getAttribute("label")}</h6>`); else { const v = $(s) ? " hidden" : "", u = s.selected ? " active" : "", r = e.multiple && s.selected ? " disabled" : "", O = s.value, q = s.textContent; l.push(`<button${v} class="dropdown-item${u}" data-dselect-value="${O}" type="button" onclick="dselectUpdate(this, '${a}', '${b}')"${r}>${q}</button>`) } return l = l.join(""), l } function z() {
    const n = e.multiple ? ' data-bs-auto-close="outside"' : "", l = Array.from(e.classList).filter(u => u !== "form-select" && u !== "form-select-sm" && u !== "form-select-lg").join(" "), s = y && !e.multiple ? `
    ...
    `; B(), e.insertAdjacentHTML("afterend", v)
  } z(); function N() { const n = e.nextElementSibling, l = n.getElementsByClassName(b)[0], s = n.getElementsByClassName("dselect-items")[0]; l.innerHTML = L(e.options, e.multiple), s.innerHTML = T(e.querySelectorAll("*")), e.multiple || (l.dataset.dselectText = C(e.options)) } e.addEventListener("change", N)
}
