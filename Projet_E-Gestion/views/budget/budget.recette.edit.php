...
$budgets = $structure->getRecetteBudgetsByStructure2($userId, $searchQuery, 20); // Retrieve recette budgets with search and limit
...
<main id="main" class="main">
    ...
    <div class="pagetitle">
        <h1>BUDGETS DES RECETTES</h1>
        <nav>
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="#">Accueil</a></li>
                <li class="breadcrumb-item active">Budgets des Recettes</li>
            </ol>
        </nav>
    </div><!-- End Page Title -->
    ...
    <section class="section dashboard">
        <div class="row">
            ...
            <button type="button" class="btn btn-primary mb-3" data-bs-toggle="modal" data-bs-target="#addBudgetModal">
                    <i class="bi bi-plus"></i> Nouveau Budget de Récette
                </button>
            ...
            <div class="col-lg-12">
                <div class="row">
                    <!-- Table budget recette -->
                    <div class="col-12">
                        <div class="card overflow-auto">
                            <div class="card-body">
                                <h5 class="card-title">
                                    Gestion des budgets des recettes
                                </h5>
                                    ...
                                    <tbody id="budgetTableBody">
                                        ...
                                        foreach ($budgets as $budget) {
                                            ...
                                            <tr>
                                                ...
                                                <td>{$budget['annee']}</td>
                                                <td>{$budget['solde_b_recette']}</td>
                                                <td>
                                                    <!-- Bouton pour modifier le budget -->
                                                    <button class='btn btn-sm btn-warning' onclick='editBudget(
                                                        {$budget['idBudget_recette_structure']}, 
                                                        \"{$budget['designation']}\",
                                                        \"{$budget['annee']}\",
                                                        \"{$budget['solde_b_recette']}\"
                                                    )'>
                                                        <i class='bi bi-pencil-square'></i> Modifier
                                                    </button>

                                                    <!-- Bouton pour afficher les utilisateurs -->
                                                    <button class='btn btn-sm btn-info' data-bs-toggle='collapse' data-bs-target='#collapseUsers{$budget['idBudget_recette_structure']}' aria-expanded='false' aria-controls='collapseUsers{$budget['idBudget_recette_structure']}'>
                                                        <i class='bi bi-people'></i> Utilisateurs
                                                    </button>

                                                    <!-- Bouton pour ajouter un utilisateur -->
                                                    <button class='btn btn-sm btn-success' data-bs-toggle='modal' data-bs-target='#modalAddUser{$budget['idBudget_recette_structure']}'>
                                            ...
                                            <tr class='collapse-row'>
                                                <td colspan='5' class='p-0'>
                                                    <div class='collapse' id='collapseUsers{$budget['idBudget_recette_structure']}'>
                                            ...
                                            $users = $structure->getUsersByBudgetRecette($budget['idBudget_recette_structure']);
                                            foreach ($users as $user) {
                                                echo "
                                                                <tr>
                                                                    <td>{$user['nomUser']}</td>
                                                                    <td>
                                                                        <a class='btn btn-sm btn-danger' onclick='confirmDeleteUser({$user['idUser_budget_recette']})'>
                                            ...
                                            echo "
                                            <div class='modal fade' id='modalAddUser{$budget['idBudget_recette_structure']}' tabindex='-1' role='dialog' aria-labelledby='modalAddUserLabel{$budget['idBudget_recette_structure']}' aria-hidden='true' data-bs-backdrop='static'>
                                                <div class='modal-dialog modal-dialog-centered'>
                                                    <div class='modal-content'>
                                                        <div class='modal-header'>
                                                            <h5 class='modal-title'>Ajouter un utilisateur au budget</h5>
                                                            <button type='button' class='btn-close' data-bs-dismiss='modal' aria-label='Close'></button>
                                                        </div>
                                                        <div class='modal-body'>
                                                            <form method='POST' action='controller/addUserToBudget.php' class='needs-validation' novalidate>
                                                                <input type='hidden' name='idBudgetRecette' value='{$budget['idBudget_recette_structure']}'>
                                                                <div class='row mb-3'>
                                                                    <div class='col-md-12'>
                                                                        <label for='userId' class='form-label'>Utilisateur</label>
                                                                        <select name='userId' class='form-select' required>
                                                                            <option value=''>Sélectionner un utilisateur</option>";
                                                                            // Fetch and display users who can be added
                                                                            $allUsers = $structure->getUsers(); // Assuming a method to get all users
                                            ...
                                            echo "
                                                                ...
                                                                <div class='modal-footer'>
                                                                    ...
                                                                    <button type='submit' name='addUserBtn' class='btn btn-primary'><i class='bi bi-save'></i> Ajouter</button>
                                                                </div>
                                                            </form>
                                                        </div>
                                                    </div>
...
<div class="modal fade" id="addBudgetModal" tabindex="-1" aria-labelledby="addBudgetModalLabel" aria-hidden="true" data-bs-backdrop="static">
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="addBudgetModalLabel">Ajouter un Budget de Récette</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form id="addBudgetRecetteForm" action="controller/create_budget_recette.php" method="POST" class="needs-validation" novalidate>
                    <div class="row mb-3">
                        <div class="col-md-6">
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="invalid-feedback">Veuillez saisir une année.</div>
                        </div>
                    </div>
                    <div class="row mb-3">
                        <div class="col-md-6">
                            <label for="solde_b_recette" class="form-label">Solde Initial <span class="text-danger">*</span></label>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form method="POST" action="controller/edit_budget.php" class="needs-validation" novalidate>
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Annuler</button>
                        <button type="submit" name="editBudgetBtn" class="btn btn-primary">
...
<script>
    ...
    document.getElementById('searchInput').addEventListener('keydown', function(event) {
        if (event.key === 'Enter') {
            ...
            window.location.href = 'budget/budget.recette.edit?search=' + encodeURIComponent(searchValue);
        }
    });
</script>
