...
// Check if the sortie ID is provided
if (!isset($_GET['id']) || empty($_GET['id'])) {
    die("Invalid sortie ID.");
}

$sortieId = intval($_GET['id']);
$structure = new Structure();

// Fetch sortie details
$sortie = $structure->getSortieById($sortieId);
$details = $structure->getDetailsSortieByManifest($sortieId);

if (!$sortie) {
    die("Sortie not found.");
}

// Fetch structure details
$structureDetails = $structure->getStructureById($sortie['Structure_idStructure']);
...
$uniqueNumber = sprintf("SORTIE-%05d", $sortieId);
...
<body>
    <div class="container">
        ...
        <hr/>
        <h1>Sortie du Dépôt N°'.$uniqueNumber.'</h1>
        <table>
            <tr><th>Date</th><td>' . htmlspecialchars(date('d/m/Y', strtotime($sortie['dateSortie'])), ENT_QUOTES, 'UTF-8') . '</td>
            <th>Transporteur</th><td>' . htmlspecialchars($sortie['transporteur'], ENT_QUOTES, 'UTF-8') . '</td></tr>
            <tr><th>Référence Document</th><td>' . htmlspecialchars($sortie['reference_document'], ENT_QUOTES, 'UTF-8') . '</td>
            <th>Dépôt</th><td>' . htmlspecialchars($structureDetails['designation'], ENT_QUOTES, 'UTF-8') . '</td></tr>
            <tr><th>Enregistré Par</th><td>' . htmlspecialchars($sortie['nomUser'], ENT_QUOTES, 'UTF-8') . '</td></tr>
        </table>
        <table>
...
try {
    ...
    $html2pdf->output('sortie_depot.pdf', 'I');
} catch (Exception $e) {
